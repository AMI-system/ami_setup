...
######### Old version using the birdconfig.py file as the config file 
# ===========================================================================================================================
...
# Make directory path name (use directory specified by user as the one where they want the audio files to be stored)
# Match year_month_day format
#path_to_file_storage = str(birdconfig.directory_to_save_audio + "%s_%s_%s" % (date_and_time.year, date_and_time.month, date_and_time.day)) # e.g. '/media/bird-pi/PiImages/BIRD/raw_audio/2023_2_8'
# Create directory with this name
#Path(path_to_file_storage).mkdir(exist_ok=True) # If exists already, then doesn't throw an error
...
# Match LID_x__SID_x__HID_x__year_month_day__hour_minute_second format
#file_to_store = str(birdconfig.LID + "__" + birdconfig.SID + "__" + birdconfig.HID + "__%s_%s_%s__%s_%s_%s.wav" % (date_and_time.year, date_and_time.month, date_and_time.day, date_and_time.hour, date_and_time.minute, date_and_time.second))
...
# For reference:
# (Can change these settings using the birdconfig.py file)
# -D plughw:1,0 is the recording Device
# -c 1 is the number of channels, here 1
# -d 60 is the duration, here 60 seconds 
# -r 22000 is the sampling rate, here 22000Hz (needs to be at least double the highest frequency bird call we want to sample)
# -f S32_LE is the format, here S32_LE
# -t wav is the file type to save, here wav file
# -V mono is the type, here mono (could also be stereo)
# -v is verbose 
...
# Arguments set at default
#proc_args = ['arecord', '-D', 'plughw:1,0', '-c', '1', '-d', '60', '-r', '22000', '-f', 'S32_LE', '-t', 'wav', '-V', 'mono', '-v', full_path]

# Arguments read in from the birdconfig.py module (which can be altered)
#proc_args = ['arecord', '-D', birdconfig.device_name, '-c', birdconfig.number_of_channels, '-d', birdconfig.duration, '-r', birdconfig.sampling_rate, '-f', birdconfig.data_format, '-t', birdconfig.file_type, '-V', birdconfig.recording_type, '-v', full_path]
